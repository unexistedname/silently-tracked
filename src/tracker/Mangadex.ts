...
export async function chapter(url: string): Promise<string[]> {
  ...
  try {
    ...
    if (res.data.result != "ok") {
      throw new Error(
        "[ MANGADEX.CHAPTER ] Error status: ",
        res.data.errors?.status,
      );
    }
...
export async function metadata(url: string): Promise<metadata> {
  try {
    ...
    if (res.data.result != "ok") {
      throw new Error(
        "[ MANGADEX.METADATA ] Error status: ",
        res.data.errors?.status,
      );
    }
    ...
    const authorID = rel.find((auth: any) => auth.type === "author").id;
    const artistID = rel.find((auth: any) => auth.type === "artist").id;
    const coverID = rel.find((cover: any) => cover.type === "cover_art").id;
    ...
    const genre = data.tags
      .map((tag: any) => tag.attributes?.name?.en.toLowerCase())
      .filter(Boolean);
